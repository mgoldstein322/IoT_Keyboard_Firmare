...
#define LED_TX_INIT    (DDRF  |=  (1<<1))
#define LED_TX_ON      (PORTF |=  (1<<1))
#define LED_TX_OFF     (PORTF &= ~(1<<1))
#define LED_TX_TOGGLE  (PORTF ^=  (1<<1))
...
#    endif
    ...
    while (1) {
        /*
        // Send Bluetooth data periodically
        uint8_t data = serial_recv();
        if (data == 0x63) LED_TX_TOGGLE;
        serial_send('!');
        wait_ms(100);
        */
